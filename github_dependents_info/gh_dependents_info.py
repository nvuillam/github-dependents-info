...
# Markers surrounding the AI summary in generated markdown, so a previous summary
# can be found again and reused when a new one can not be generated.
LLM_SUMMARY_START_MARKER = "<!-- gh-dependents-info-summary-start -->"
LLM_SUMMARY_END_MARKER = "<!-- gh-dependents-info-summary-end -->"
...
class GithubDependentsInfo:
    def __init__(self, repo, **options) -> None:
        ...
        self.llm_summary_reused: bool = False
    ...
    def _extract_summary_from_markdown_content(self, content: str) -> str | None:
        """Extract a summary previously written by github-dependents-info in a markdown content."""
        marked = re.search(
            re.escape(LLM_SUMMARY_START_MARKER) + "(.*?)" + re.escape(LLM_SUMMARY_END_MARKER),
            content,
            re.DOTALL,
        )
        if marked is not None:
            ...
            # Markdown generated before summary markers existed: fallback on the "## Summary" section
            heading = re.search(r"^##\s+Summary\s*$", content, re.MULTILINE)
            if heading is None:
                return None
            block = content[heading.end() :]
        block = re.sub(r"^\s*##\s+Summary\s*$", "", block, count=1, flags=re.MULTILINE)
        # Keep only the summary text: stop at the next heading, badge, table or footer
        summary_lines: list[str] = []
        for line in block.splitlines():
            stripped = line.strip()
            if stripped.startswith(("#", "[![", "|", "_Generated using [")):
                break
            summary_lines.append(line)
        return "\n".join(summary_lines).strip() or None

    def _load_previous_markdown_summary(self) -> str | None:
        """Read the summary generated by a previous run in the output markdown file."""
        if not self.markdown_file:
            return None
        try:
            if not os.path.isfile(self.markdown_file):
                return None
            with open(self.markdown_file, encoding="utf-8") as f:
                content = f.read()
        except (OSError, UnicodeDecodeError) as exc:
            logger.warning("Failed to read previous summary from %s: %s", self.markdown_file, exc)
            return None
        return self._extract_summary_from_markdown_content(content)

    def reuse_previous_llm_summary(self, reason: str) -> bool:
        """Reuse the summary of a previous run when a new one can not be generated.

        This avoids losing an existing summary when the LLM call is impossible
        (no API key anymore, no credit left, rate limit, timeout...).
        """
        if self.llm_summary:
            return True
        if self.load_llm_summary():
            self.llm_summary_reused = True
        else:
            previous_summary = self._load_previous_markdown_summary()
            if not previous_summary:
                return False
            self.llm_summary = previous_summary
            self.llm_summary_reused = True
            # Keep it in the progress directory so next runs can find it faster
            self.save_llm_summary()
        logger.warning(
            "Unable to generate a new AI summary (%s): reusing the summary generated by a previous run",
            reason,
        )
        return True
    ...
    async def maybe_generate_llm_summary(self) -> None:
        ...
        if provider_info is None:
            self.reuse_previous_llm_summary("no LLM API key found in environment variables")
            return
        ...
        try:
            ...
            if content:
                ...
                self.llm_summary_error = "LLM returned an empty summary"
                self.reuse_previous_llm_summary(self.llm_summary_error)
            ...
            self.reuse_previous_llm_summary(str(exc))
        finally:
    ...
    def _append_llm_summary(self, md_lines: list) -> None:
        """Append the AI summary section, surrounded by markers so it can be reused by later runs."""
        if not self.llm_summary:
            return
        md_lines += [
    ...
    def _build_single_markdown(self, **options) -> str:
        ...
        self._append_llm_summary(md_lines)
